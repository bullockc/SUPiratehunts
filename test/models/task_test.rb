...
class TaskTest < ActiveSupport::TestCase
  ...
  test "task is complete" do
  	task = Task.create(task_type: '1', points: '25', prompt: 'Sample task', clue: 'Try to complete', correct_answer: 'Apple', hunt: '1234')
  ...
  test "task without type" do
  	task = Task.create(points: '25', prompt: 'Sample task', clue: 'Try to complete', correct_answer: 'Apple', hunt: '1234')
  ...
  test "task without points" do
  	task = Task.create(task_type: '1', prompt: 'Sample task', clue: 'Try to complete', correct_answer: 'Apple', hunt: '1234')
  	assert_not task.valid?, 'Should be invalid without all parameters'
  end

  #test that the entry is invalid when the prompt is missing
  test "task without prompt" do
  	task = Task.create(task_type: '1', points: '25', clue: 'Try to complete', correct_answer: 'Apple', hunt: '1234')
  	assert_not task.valid?, 'Should be invalid without all parameters'
  end

  #test that the entry is invalid when the clue is missing
  test "task without clue" do
  	task = Task.create(task_type: '1', points: '25', prompt: 'Sample task', correct_answer: 'Apple', hunt: '1234')
  	assert_not task.valid?, 'Should be invalid without all parameters'
  end

  #test that the entry is invalid when the correct answer is missing
  test "task without correct answer" do
  	task = Task.create(task_type: '1', points: '25', prompt: 'Sample task', clue: 'Try to complete', hunt: '1234')
  	assert_not task.valid?, 'Should be invalid without all parameters'
  end
